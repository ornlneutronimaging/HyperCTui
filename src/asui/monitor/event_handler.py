...
class EventHandler:
		...
		if len(list_folders) == 0:
			...
			for _row in np.arange(nbr_obs_expected):
				...
				if item in list_folders:
					...
					self.parent.dict_ob_log_err_metadata[_row] = {'file_name': new_ob,
																  'log_file': log_file,
																  'err_file': err_file,
	...
	def checking_status_of_expected_projections(self):
		...
		if len(list_folders) > nbr_projections_expected:
			message = "Output projections folder contains already more data than requested, check the validity of the folder!"
			show_status_message(parent=self.grand_parent,
								message=message,
								status=StatusMessageStatus.error,
								duration_s=10)
		...
		if len(list_folders) == 0:
			...
			# so we need to show that the acquisition of the first one started
			...
			o_get = GetMonitor(parent=self.parent,
							   grand_parent=self.grand_parent)
			its_first_row_with_measuring_projections = True
			for _row in np.arange(nbr_projections_expected):
				item = o_table.get_item_str_from_cell(row=_row,
													  column=0)
				if item in list_folders:
					...
					if its_first_row_with_measuring_projections:
						message = DataStatus.in_progress
						color = IN_PROGRESS
						its_first_row_with_measuring_projections = False
					else:
						message = DataStatus.in_queue
						color = IN_QUEUE
					o_table.insert_item(row=_row,
					...
					self.parent.dict_ob_log_err_metadata[_row] = {'file_name': new_projection,
																  'log_file': log_file,
																  'err_file': err_file,
																  'metadata_file': metadata_file}
